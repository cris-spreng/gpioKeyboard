...
int inputPins[15]     = { 0        , 1        , 18       , 19       , 20       , 21       , 22       , 23       , 5        , 6        , 9        , 10       , 11       , 12       , 13        };
int inputKeycodes[15] = { HID_KEY_Q, HID_KEY_W, HID_KEY_E, HID_KEY_R, HID_KEY_T, HID_KEY_A, HID_KEY_S, HID_KEY_D, HID_KEY_F, HID_KEY_G, HID_KEY_Z, HID_KEY_X, HID_KEY_C, HID_KEY_V, HID_KEY_B };
...
void loop(void)
{
  ...
  if ( ble.isConnected() )
  {
    ...
    {
      // GPIO is active low     
      if ( digitalRead(inputPins[i]) == LOW && count < 6)
      {
        keyReport.keycode[count] = inputKeycodes[i];
        count++;
      }
      if(count >= 6){
        break;
